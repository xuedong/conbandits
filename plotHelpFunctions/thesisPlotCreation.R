obtainThesisTimestepPlot = function(performanceDataTable, plotTitle = NULL, benchmarkPerf =  NULL){
  ...
  #  resPlot = resPlot + geom_hline(mapping = aes(colour = "VBS", yintercept = vbsPerf), size = lineSize) 
  #  resPlot = resPlot + geom_hline(mapping = aes(colour = "singleBest", yintercept = sbsPerf), size = lineSize) 
  # resPlot = resPlot + scale_colour_manual(values=c("red", "green")) 
